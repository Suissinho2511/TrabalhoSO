...
int main(int argc, char **argv)
{
	if (argc > 1){
		...
		else if (strcmp(argv[1], "transform") == 0)
		{
			...
			printf("* ./aurras status * -> to get info of the server \n");
			printf("* ./aurras transform input-filename output-filename filter-id-1 filter-id-2 ... * -> to aplly filter to a audio \n");
			return 0;
		}
		else
		{
		...
		printf("* ./aurras help * -> to get help \n");
		return 0;
	}
}
